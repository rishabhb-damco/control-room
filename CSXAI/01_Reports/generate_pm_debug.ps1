...
function TBox($sl,$l,$t,$w,$h,$txt,$sz,$col,$bold=$false,$align=1,$fn="Aptos",$va=1){ Write-Host "TBox: sz=[$sz] txtType=[$($txt.GetType().Name)] txt=[$($txt.ToString().SubString(0,[Math]::Min(30,$txt.ToString().Length)))]"
...
foreach($ph in $phases){
    $x=$ph[0]; $y=$ph[1]; $label=$ph[2]; $title=$ph[3]; $bgc=$ph[4]; $acc=$ph[5]; $desc=$ph[6]
